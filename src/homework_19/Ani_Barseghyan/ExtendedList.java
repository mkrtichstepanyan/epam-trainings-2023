...
public class ExtendedList<T> extends ArrayList<T> {
    ...
    public List<ExtendedList<T>> partition(Predicate<T> predicate) {
        ExtendedList<T> left = this.stream().filter(predicate).collect(Collectors.toCollection(ExtendedList::new));
        ExtendedList<T> right = this.stream().filter(predicate.negate()).collect(Collectors.toCollection(ExtendedList::new));
