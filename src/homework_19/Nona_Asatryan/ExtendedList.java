...
public class ExtendedList<T> extends ArrayList<T> {
    ...
    public List<ExtendedList<T>> partition(Predicate<T> predicate) {
        ...
        partitions.add(stream().filter(predicate).collect(Collectors.toCollection(ExtendedList::new)));
        partitions.add(stream().filter(predicate.negate()).collect(Collectors.toCollection(ExtendedList::new)));
        return partitions;
    }
