...
public class ExtendedList<T> extends ArrayList<T> {
    ...
    public void fill(Supplier<T> supplier, int count) {
        Stream<T> limit = Stream.generate(supplier).limit(count);
        limit.forEach((n)->this.add(n));
    }
    ...
    public List<ExtendedList<T>> partition(Predicate<T> predicate) {
        ...
        partitions.add(this.stream().filter(predicate).collect(Collectors.toCollection(ExtendedList::new)));
        partitions.add(this.stream().filter(predicate.negate()).collect(Collectors.toCollection(ExtendedList::new)));
        return partitions;
