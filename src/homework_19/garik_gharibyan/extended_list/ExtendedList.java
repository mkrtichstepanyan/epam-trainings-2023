...
public class ExtendedList<T> extends ArrayList<T> {
    ...
    public <R> ExtendedList<R> map(Function<T, R> function) {
        ...
        ExtendedList<R> newExtendedList;
...
//        }
    ...
    public void fill(Supplier<T> supplier, int count) {
        ...
        Stream<Supplier<T>> stream = Stream.generate(() -> supplier);
        stream.limit(count).forEach(mySupplier -> this.add(mySupplier.get()));
    }
    ...
    public List<ExtendedList<T>> partition(Predicate<T> predicate) {
        ExtendedList<T> list1;
        ExtendedList<T> list2;
...
//        }
        ...
        list1 = this.stream().filter(predicate).collect(Collectors.toCollection(ExtendedList::new));
        list2 = this.stream().filter(predicate.negate()).collect(Collectors.toCollection(ExtendedList::new));
