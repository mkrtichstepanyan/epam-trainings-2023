...
public class Order {
    ...
    public void addPizza(String pizzaName, PizzaType type, Ingredient[] ingredients, int quantity) throws OrderException {
        if (quantity > MAX_PIZZA_AMOUNT ) {
            throw new OrderException("You cannot order more than 10 pizzas.");
        }
    ...
    public void addDrink(String drinkName, DrinkType type, int quantity) throws InvalidPriceException {
        if (quantity <= 0) {
            throw new InvalidPriceException("Quantity cannot be negative or zero");
        }
        if (type.getPrice() <= 0) {
            throw new InvalidPriceException("The price cannot be negative or zero.");
        }
